        ...
        methods: {
            ...
            listenEnd (index, event) {
                if (event.type === 'mouseup') {
                    ...
                    let distX = event.clientX - this.mousedown.startX
                    let distY = event.clientY - this.mousedown.startY
                    onEndAction(this, index, event, delta, distX, distY)
                } else if (event.type === 'touchend') {
                    let delta = event.timeStamp - this.touchstart.startTime
                    let distX = this.touchend && this.touchend.endX ? this.touchend.endX - this.touchstart.startX : 0
                    let distY = this.touchend && this.touchend.endY ? this.touchend.endY - this.touchstart.startY : 0
